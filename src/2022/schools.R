...
files <- tempfile()
unzip("data/2022/SCH_QQQ_SPSS.zip", exdir = files, unzip = "/usr/bin/unzip")
...
raw <- read_sav(file.path(files, "CY08MSP_SCH_QQQ.sav"), user_na = TRUE)
...
write_parquet(processed, "build/2022/schools.parquet",
  compression = "zstd", compression_level = 10
)

write_parquet(flags, "build/2022/flags/schools.parquet",
  compression = "zstd", compression_level = 10
)
